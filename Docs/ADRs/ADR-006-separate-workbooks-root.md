...
`Config/Templates/Config.template.ini`). When set, workbooks are written to that directory instead
...
- **Always write workbooks to a fixed local path** -- simpler, no parameter needed.
  ...
  portability across machines sharing the same `Config/Templates/Config.template.ini`.
...
- Two directory variables must be kept consistent in `Config.ini`; setup documentation
  must explain both.
